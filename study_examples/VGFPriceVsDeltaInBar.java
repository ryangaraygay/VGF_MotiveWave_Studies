...
@StudyHeader(
 namespace="org.vgf", 
 id="VGF_PRICE_VS_DELTA_IN_BAR", 
 name="VGF Price-Delta Convergence In Bar",
 label="VGF Price-Delta Convergence",
 desc="",
 menu="VGF Studies",
 overlay=true,
 studyOverlay=false)
public class VGFPriceVsDeltaInBar extends Study
{
  enum Values { DELTACLOSE2, HLEMA1, DCEMA1, HLEMA2, HLEMA3, DCEMA2, DCEMA3, HLTEMA, DCTEMA, CORR, PDTEMA, CONVDIV };

  enum Names { ENABLED, MAXBARS, CONVDIVPERIOD, PDBAR, UPCONV, DOWNCONV, UPDIV, DOWNDIV, MESSYUP, MESSYDOWN, MESSYTHRESHOLD, CONVWIDTHFULL, CONVWIDTHMESSY };
  ...
  public void initialize(Defaults defaults)
  {
    ...
    convdiv.addRow(new ColorDescriptor(Names.UPCONV.toString(), "Positive Convergence", new Color(18, 212, 18, 204)));
    convdiv.addRow(new ColorDescriptor(Names.DOWNCONV.toString(), "Negative Convergence", new Color(255, 0, 0, 204)));
  ...
  protected void calculate(int index, DataContext ctx)
  {
    ...
    int lineWidth = getSettings().getInteger(Names.CONVWIDTHFULL.toString());
    if (messy && !divergence) {
      lineWidth = getSettings().getInteger(Names.CONVWIDTHMESSY.toString());
    }

    float close = series.getClose(index);
    float open = series.getOpen(index);
    Line line = new Line(new Coordinate(sTime, open), new Coordinate(sTime, close));
    line.setColor(barColor);
    line.setStroke(new BasicStroke(lineWidth));
    addFigure(line);
